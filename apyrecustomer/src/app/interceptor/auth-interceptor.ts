...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    if (req.url.includes('/get-magic-link') || req.url.includes('/verify-magic-link') || req.url.includes('/exchange-magic-link')) {
...
//         tap((res) => { 
//           console.log('f')
//           console.log('THis is the new refresh token recieved from backend' + res.headers.get('refresh-token'))
//           this.tokenSvc.setRefreshToken(res.headers.get('refresh-token'))
//         }),
//         switchMap((refreshedToken: string | null) => { // retry  failed request with new access token
//           if (refreshedToken) {
